...
# Para executar: python grafico_acelerometro_interativo.py
...
current_start = df['time'].min()  # Início atual da janela
current_end = current_start + timedelta(minutes=window_minutes)  # Fim atual da janela

# Criar figura e eixos
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10))
fig.suptitle('Dados do Acelerômetro - Janela de 10 minutos', fontsize=16, fontweight='bold')
...
def update_plot():
    ...
    if len(df_window) > 0:
        ...
        ax1.set_title(f'Aceleração X, Y e Z - {current_start.strftime("%H:%M")} até {current_end.strftime("%H:%M")}', fontsize=14)
        ax1.legend(loc='best')
        ...
        ax1.text(0.5, 0.5, 'Nenhum dado nesta janela', transform=ax1.transAxes, 
                ha='center', va='center', fontsize=14)
        ax2.text(0.5, 0.5, 'Nenhum dado nesta janela', transform=ax2.transAxes, 
                ha='center', va='center', fontsize=14)
...
def anterior(event):
    ...
    if new_start >= df['time'].min():
        ...
        print("Já está no início dos dados!")
...
def proximo(event):
    ...
    if new_end <= df['time'].max():
        ...
        print("Já está no fim dos dados!")
...
# Atualizar layout para dar espaço aos botões
...
print("\n" + "="*60)
print("Controles de Navegacao:")
print("  Anterior: Volta 5 minutos")
print("  Proximo: Avanca 5 minutos")
print("  Inicio: Vai para o inicio dos dados")
print("  Fim: Vai para o fim dos dados")
print("="*60)
print("\nUse os botoes na parte inferior da janela para navegar.")
print("A janela sempre mostra 10 minutos de dados.")
