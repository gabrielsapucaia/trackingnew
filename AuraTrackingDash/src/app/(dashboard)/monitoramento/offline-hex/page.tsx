...
export default function MonitoramentoOfflineHexPage() {
  ...
  const [radiusMeters, setRadiusMeters] = useState("40")
  ...
  const radiusValue = useMemo(() => parseFloat(radiusMeters) || 40, [radiusMeters])
    ...
    <div className="grid gap-4 items-start" style={{ gridTemplateColumns: "12% 1fr" }}>
      <aside className="rounded-lg border bg-card p-4 shadow-sm sticky top-4 self-start">
        <div className="space-y-3">
          ...
          <div className="pt-2 space-y-2 border-t border-white/10">
            <p className="text-sm font-semibold">Heatmap (hex)</p>
            <label className="flex items-center gap-2 text-sm text-white">
              <span>Raio (m)</span>
              <input
                type="number"
