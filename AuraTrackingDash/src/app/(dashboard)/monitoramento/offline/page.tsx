...
export default function MonitoramentoOfflinePage() {
  ...
  // Reset defaults when page mounts
  useEffect(() => {
  ...
  useEffect(() => {
    const loadEquipments = async () => {
      ...
      if (data) {
        const mapped = data.map((e: any) => ({ id: e.id, tag: e.tag }))
  ...
  const handleLoadHistory = async () => {
    try {
      // Ajusta fuso -3 para UTC (offset +03:00 em relação ao Brasil)
  ...
  const speedConfig = useMemo(() => {
    ...
    return {
      ...
      colors: [
        [34, 197, 94, 220], // verde
        [234, 179, 8, 220], // amarelo
        [249, 115, 22, 220], // laranja
        [239, 68, 68, 220], // vermelho
