...
import { AlertTriangle, CalendarRange, Clock, HardHat, MapPin, Timer } from "lucide-react"
...
export default function MonitoramentoOfflineGridLayerCopyPage() {
  ...
  const [cellSizeMeters, setCellSizeMeters] = useState("40")
  const [opacity, setOpacity] = useState("0.8")
  const [gridHydrated, setGridHydrated] = useState(false)
  const [useContinuous, setUseContinuous] = useState(false)
  ...
  useEffect(() => {
    ...
    const now = new Date()
    setDateStart(formatDate(now))
    setDateEnd(formatDate(now))
    setTimeEnd(formatTime(now))
    setTimeStart(formatTime(new Date(now.getTime() - 60 * 60 * 1000)))
    setHydrated(true)
    setSpeedHydrated(true)
    setGridHydrated(true)
  }, [])
  ...
  const devicesToShowRaw = historicalDevices.length > 0 ? historicalDevices : filteredDevices
  
  const devicesToShow = useMemo(() => {
    return devicesToShowRaw.filter(d => {
      const transmissionMode = d.transmissionMode || "online"
      if (!showQueued && transmissionMode === "queued") return false
      if (!showOnline && transmissionMode === "online") return false
      return true
    })
  }, [devicesToShowRaw, showQueued, showOnline])
  
  const transmissionStats = useMemo(() => {
    const online = devicesToShowRaw.filter(d => (d.transmissionMode || "online") === "online").length
    const queued = devicesToShowRaw.filter(d => (d.transmissionMode || "online") === "queued").length
    const total = devicesToShowRaw.length
    return { 
      online, 
      queued, 
      total, 
      queuedPercent: total > 0 ? ((queued / total) * 100).toFixed(1) : "0" 
    }
  }, [devicesToShowRaw])
  ...
  const cellSizeValue = useMemo(() => parseFloat(cellSizeMeters) || 40, [cellSizeMeters])
  const opacityValue = useMemo(() => parseFloat(opacity) || 0.8, [opacity])

  const bucketStats = useMemo(() => {
    const speeds = devicesToShow
      .map((d) => (typeof d.speedKmh === "number" && Number.isFinite(d.speedKmh) ? d.speedKmh : null))
      .filter((v): v is number => v !== null)
    const [b0, b1, b2] = speedConfig.breakpoints
    let green = 0
    let yellow = 0
    let orange = 0
    let red = 0
    let min = speeds.length ? Math.min(...speeds) : null
    let max = speeds.length ? Math.max(...speeds) : null
    ...
    return { green, yellow, orange, red, min, max, total: speeds.length }
  }, [devicesToShow, speedConfig.breakpoints])
    ...
    <div className="grid gap-4 items-start" style={{ gridTemplateColumns: "12% 1fr" }}>
      <aside className="rounded-lg border bg-card p-4 shadow-sm sticky top-4 self-start">
        <div className="space-y-3">
          <div>
            <p className="text-sm font-semibold">Offline (GridLayer) 2</p>
            <p className="text-xs text-muted-foreground">
              Cópia para testes: grids em metros, cores por velocidade média.
            </p>
          </div>
          ...
          <div className="pt-2 space-y-2 border-t border-white/10">
            <p className="text-sm font-semibold">Grid (metros)</p>
            <label className="flex items-center gap-2 text-sm text-white">
              <span>Célula (m)</span>
              <input
                type="number"
                className="w-full rounded bg-background px-2 py-1 text-xs outline-none"
                value={gridHydrated ? cellSizeMeters : ""}
            ...
            <label className="flex items-center gap-2 text-sm text-white">
              <input
                type="checkbox"
                checked={autoCell}
                onChange={(e) => setAutoCell(e.target.checked)}
                className="h-4 w-4 accent-primary"
              />
              <span>Ajustar célula automaticamente (zoom)</span>
            </label>
            <label className="flex items-center gap-2 text-sm text-white">
              <input
                type="checkbox"
                checked={useContinuous}
                onChange={(e) => setUseContinuous(e.target.checked)}
                className="h-4 w-4 accent-primary"
              />
              <span>Usar escala contínua (interpolar pelas faixas)</span>
            </label>
            <label className="flex items-center gap-2 text-sm text-white">
              <span>Opacidade</span>
              <input
          ...
          <div className="pt-2 space-y-2 border-t border-white/10">
            <p className="text-sm font-semibold">Modo de Transmissão</p>
            <label className="flex items-center gap-2 text-sm text-white">
              <input
                type="checkbox"
                checked={showOnline}
            ...
            {transmissionStats.total > 0 && (
              <div className="text-xs text-muted-foreground pt-1">
                <p>Online: {transmissionStats.online} ({((transmissionStats.online / transmissionStats.total) * 100).toFixed(1)}%)</p>
                <p>Queued: {transmissionStats.queued} ({transmissionStats.queuedPercent}%)</p>
              </div>
            )}
          </div>

          <div className="pt-2 space-y-2 border-t border-white/10">
            <p className="text-sm font-semibold">Opacidade do mapa</p>
            <label className="flex items-center gap-2 text-sm text-white">
              <span>Base</span>
              <input
                type="number"
                ...
                max="1"
                className="w-full rounded bg-background px-2 py-1 text-xs outline-none"
                value={gridHydrated ? basemapOpacity : ""}
                onChange={(e) => setBasemapOpacity(e.target.value)}
                suppressHydrationWarning
              />
            </label>
            <label className="flex items-center gap-2 text-sm text-white">
              ...
              <input
                ...
                max="1"
                className="w-full rounded bg-background px-2 py-1 text-xs outline-none"
                value={gridHydrated ? orthoOpacity : ""}
                onChange={(e) => setOrthoOpacity(e.target.value)}
                suppressHydrationWarning
              />
            </label>
          </div>
        ...
        <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
          <div className="p-2 sm:p-4">
            <OfflineMap
              ...
              gridOptions={{
                cellSizeMeters: cellSizeValue,
                opacity: opacityValue,
                autoAdjustCell: autoCell,
                useContinuous,
              }}
              basemapOpacity={parseFloat(basemapOpacity) || 1}
              orthoOpacity={parseFloat(orthoOpacity) || 1}
            />
          </div>
        </div>

        <div className="rounded-lg border bg-card p-4 shadow-sm text-xs text-white/90 space-y-1">
          <p className="font-semibold text-sm">Buckets atuais (km/h)</p>
          <p>
            Verde: {bucketStats.green} · Amarelo: {bucketStats.yellow} · Laranja: {bucketStats.orange} · Vermelho:{" "}
            {bucketStats.red}
          </p>
          <p>
            Velocidade min: {bucketStats.min ?? "—"} · max: {bucketStats.max ?? "—"} · pontos: {bucketStats.total}
          </p>
        </div>
