...
export default function MonitoramentoOfflineHeatmapPage() {
  ...
  const [radiusPixels, setRadiusPixels] = useState("40")
  ...
  const radiusValue = useMemo(() => parseFloat(radiusPixels) || 40, [radiusPixels])
  const intensityValue = useMemo(() => parseFloat(intensity) || 1, [intensity])
    ...
    <div className="grid gap-4 items-start" style={{ gridTemplateColumns: "12% 1fr" }}>
      <aside className="rounded-lg border bg-card p-4 shadow-sm sticky top-4 self-start">
        <div className="space-y-3">
          ...
          <div className="pt-2 space-y-2 border-t border-white/10">
            <p className="text-sm font-semibold">Heatmap</p>
            <label className="flex items-center gap-2 text-sm text-white">
              <span>Raio (px)</span>
              <input
                type="number"
                className="w-full rounded bg-background px-2 py-1 text-xs outline-none"
                value={radiusPixels}
                onChange={(e) => setRadiusPixels(e.target.value)}
              />
            </label>
            <label className="flex items-center gap-2 text-sm text-white">
