...
export default function MonitoramentoOfflineGridPage() {
  ...
  const [cellSizePixels, setCellSizePixels] = useState("40")
  ...
  const cellSizeValue = useMemo(() => parseFloat(cellSizePixels) || 40, [cellSizePixels])
    ...
    <div className="grid gap-4 items-start" style={{ gridTemplateColumns: "12% 1fr" }}>
      <aside className="rounded-lg border bg-card p-4 shadow-sm sticky top-4 self-start">
        <div className="space-y-3">
          <div>
            <p className="text-sm font-semibold">Offline (Grid)</p>
            <p className="text-xs text-muted-foreground">
              Grids coloridos pela velocidade média (km/h) usando ScreenGridLayer.
            </p>
          </div>
          ...
          <div className="pt-2 space-y-2 border-t border-white/10">
            <p className="text-sm font-semibold">ScreenGrid</p>
            <label className="flex items-center gap-2 text-sm text-white">
              <span>Célula (px)</span>
              <input
                type="number"
                className="w-full rounded bg-background px-2 py-1 text-xs outline-none"
                value={gridHydrated ? cellSizePixels : ""}
                onChange={(e) => setCellSizePixels(e.target.value)}
                suppressHydrationWarning
              />
            </label>
            <label className="flex items-center gap-2 text-sm text-white">
