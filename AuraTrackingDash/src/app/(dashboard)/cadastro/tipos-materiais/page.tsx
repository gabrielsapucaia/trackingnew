...
type SortColumn = "status" | "name" | "description" | "created_at" | "updated_at"

export default function TiposMateriaisPage() {
  ...
  const [deleting, setDeleting] = useState<TipoMaterial | null>(null)
  const [form, setForm] = useState<Omit<TipoMaterial, "id" | "created_at" | "updated_at">>({
    name: "",
    description: "",
    status: "active",
  })
  ...
  const fetchTipos = async () => {
    try {
      setLoading(true)
      setError(null)
      const { data, error } = await getTiposMateriais(false)
      if (error) throw error
      setTipos(data || [])
    } catch (err) {
      console.error("Erro ao buscar tipos de materiais:", err)
  ...
  const toggleStatus = async (id: number, current: string | null | undefined) => {
    ...
    try {
      const { data, error } = await updateTipoMaterial(id, { status: newStatus })
      if (error) throw error
  ...
  const handleAdd = async () => {
    try {
      setError(null)
      const { data, error } = await addTipoMaterial(form)
      if (error) throw error
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const { data, error } = await updateTipoMaterial(editing.id, {
        name: editing.name,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const { error } = await deleteTipoMaterial(deleting.id)
      if (error) throw error
  ...
  const statusBadge = (tipo: TipoMaterial) => {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-3xl font-bold">Tipos de Materiais</h1>
        </div>
    ...
    <div className="space-y-6">
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[460px]">
            <DialogHeader>
              ...
              <DialogDescription>Cadastre um tipo de material.</DialogDescription>
            </DialogHeader>
      ...
      {tipos.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">Cadastre o primeiro tipo de material para começar.</p>
        </div>
      ) : (
              ...
              <tbody className="bg-card divide-y divide-border">
                {sortedTipos.map((tipo) => (
                  <tr key={tipo.id} className="hover:bg-accent transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-muted-foreground max-w-sm truncate">{tipo.description || "-"}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[460px]">
          <DialogHeader>
            ...
            <DialogDescription>Atualize as informações do tipo de material.</DialogDescription>
          </DialogHeader>
          {editing && (
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
