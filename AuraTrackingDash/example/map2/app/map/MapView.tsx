...
export default function MapView({ devices, isLoading, error, connectionStatus }: MapViewProps) {
  ...
  // Selected/hovered device for tooltip/highlight
  ...
  const handleDeckClick = useCallback((info: PickingInfo) => {
    if (info.object && (info.layer?.id === 'devices-truck-icon-layer' || info.layer?.id === 'devices-label-layer')) {
  ...
  // Handle hover for tag/icon highlight
  ...
  const layers = useMemo(() => {
    ...
    return [
      // Dump truck icon layer (single icon, no circle)
      ...
      new TextLayer<DeviceSummary>({
        ...
        getSize: 11, // 20% smaller than 14
        sizeUnits: 'pixels',
        getColor: [255, 255, 255, 255],
        getTextAnchor: 'middle',
        getAlignmentBaseline: 'center',
        getPixelOffset: [0, -40], // closer to the icon
  ...
  }, [visibleDevices, validDevices, mapState.status, devicesKey, hoveredDeviceId, selectedDeviceId]);
